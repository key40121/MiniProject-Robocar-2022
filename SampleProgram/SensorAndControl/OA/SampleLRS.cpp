...
private:
	...
	void storeLRF(LrsResult res, int dev){
		if(dev == 1){
			...
			max_dist[0] = 0;
			min_dist[0] = 4000;
			for(int j = (res.data_length+44)/2 ; j < 500; j++){
			    sum = 0;
				for(int i = j - WINDOWS/2; i < j + WINDOWS/2; i++)
				{
					if( DISTANCE < res.data[i] && res.data[i] <= MAX_LENGTH ){
						sum++;
					}
				}
				printf("%d\n",sum);
				if(sum == WINDOWS)
				{
					angle = -120+240.0/(res.data_length+44)*j;
					break;
				}
				printf("%d\n",res.data_length);
				int j1 = res.data_length + 44 - j;
				sum = 0;
				for(int i = j1 - WINDOWS/2; i < j1 + WINDOWS/2; i++)
				{
					if( DISTANCE < res.data[i] && res.data[i] <= MAX_LENGTH ){
						sum++;
					}
				...
				if(sum == WINDOWS)
				{
					angle = -120+240.0/(res.data_length+44)*j1;
					break;
				}
			...
			_RcControl.SetDriveSpeed(70);

			/*for( int j= 225; j< 456; j ++){
