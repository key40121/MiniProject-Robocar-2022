...
void SlidingWindow(cv::Mat image, cv::Rect);
...
int main() {
	...
	*/
	...
	cv::Rect rect;
	rect = cv::Rect(50, 210, 60, 30);
	
	SlidingWindow(processed, rect);
	
	/*for (int i=0;pts.size(); i++)
	{
...
void SlidingWindow(cv::Mat image, cv::Rect window) {
	std::vector<cv::Point2f> points;
	...
	std::cout << "roi dims " << roi.dims << std::endl;
	std::cout << "roi channels " << roi.channels() << std::endl;
